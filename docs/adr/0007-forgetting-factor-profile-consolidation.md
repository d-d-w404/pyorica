...
- `tau_const` (and the `lambda_const` it derives) remains a floor under **all** profiles, not just `"constant"` — including `"cooling"` and `"adaptive"`. This preserves the numerical-stability guard from ADR-0006 (unconstrained λ decay under `tau_const=inf` risks the `lam_prod` divergence documented there) while still letting cooling/adaptive drive λ above that floor.
...
- **Make `tau_const`/`lambda_const` a floor only under `"constant"`, letting `"cooling"`/`"adaptive"` decay unconstrained.** Considered, but rejected in favor of keeping the floor universal: it preserves the numerical-stability protection from ADR-0006 for all profiles, and a caller who wants unconstrained decay can still get it via `tau_const=np.inf`.
