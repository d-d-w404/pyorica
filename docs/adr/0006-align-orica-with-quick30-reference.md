...
- `force_constant_lambda` defaults to `True`, which means `ff_profile="cooling"` and `"adaptive"` are effectively inert unless a caller explicitly sets `force_constant_lambda=False` — this mirrors quick30's own hard-coded behavior, not a pyorica-specific simplification.
