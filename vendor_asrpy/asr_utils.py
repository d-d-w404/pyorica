...
def fit_eeg_distribution(X, min_clean_fraction=0.25, max_dropout_fraction=0.1,
    ...
    # .item() extracts the scalar: np.diff of a 2-element array is a size-1
    # ndarray, and NumPy >=2 raises TypeError on int()/float() of such arrays
    # instead of the DeprecationWarning NumPy <2 gave (pyorica patch).
    max_width = np.diff(quants).item()
    ...
    # np.diff(opt_bounds) is a size-1 ndarray (opt_bounds has 2 elements), so
    # alpha/mu inherit that shape; .item() extracts the scalar since callers
    # assign these into scalar array slots, which NumPy >=2 no longer allows
    # for non-scalar arrays even when size 1 (pyorica patch).
