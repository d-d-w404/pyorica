...
def asr_calibrate(X, sfreq, cutoff=20, blocksize=100, win_len=0.5,
    ...
    # block_covariance has an off-by-one when (n_samples - 2) % blocksize == 0
    # that misshapes the block covariance array; trimming one sample avoids
    # it with negligible effect on calibration (pyorica patch).
    if ns >= 3 and (ns - 2) % blocksize == 0:
        X = X[:, :-1]
        nc, ns = X.shape
