...
**Forgetting factor (λ)**:
A scalar in `(0, 1)` that controls how fast ORICA adapts to non-stationary data. `tau_const` (stationarity window, seconds) only affects the `constant` profile — it does not floor `cooling`/`adaptive` (see [ADR-0008](docs/adr/0008-remove-forgetting-factor-floor-from-cooling-adaptive.md); an earlier design floored λ under all three profiles, which made cooling/adaptive numerically indistinguishable from constant under everyday near-stationary data). Profiles:
- `constant` (default) — λ is pinned to the steady-state value derived from `tau_const`; assumes data is stationary within that window. The production/real-time setting.
- `cooling` — λ decays unconstrained toward 0 over the session per `lambda_0`/`gamma`, assuming the entire session is stationary. Validation-only: used to compare against offline (batch) ICA, not for real-time deployment.
- `adaptive` — λ starts at `lambda_0` and responds to the non-stationarity index (`_Rn`) to speed up adaptation when the data's statistics shift, otherwise decaying like cooling. Enhances learning when nonstationarity happens.
...
**Classifier**:
Any callable `(data, sources, unmixing, mixing, sfreq) → artifact_mask` that identifies artifact ICs. `data` is the ASR-cleaned EEG chunk; `unmixing` is `W × sphere`; `mixing` is `pinv(unmixing)` — the mixing matrix mapping source space back to channel space. The default implementation (`ICLabelClassifier`) uses ICLabel; any callable with this signature is valid. `ICLabelClassifier` accepts both legacy label spellings (`'eog'`, `'ecg'`, `'ch_noise'`) and newer mne-icalabel aliases (`'eye'`, `'heart'`, `'channel_noise'`), and normalises EEGLAB-style uppercase channel names automatically. An IC is rejected once its top-1 label probability exceeds `threshold` AND its canonical label is in `artifact_labels` (default `{muscle, eye, heart, line_noise, channel_noise}`) — an allow-list; unrecognized labels are never rejected. `apply_car_bandpass` controls whether common-average-reference and a 1–100 Hz bandpass are applied to `data` before running ICLabel, matching its documented training assumptions (default `False`). `True` slightly improves classification accuracy (fewer brain ICs reduced as artifacts) at the cost of extra per-chunk bandpass-filter compute; CAR alone (without the bandpass) was benchmarked and found worse than applying both together.
