...
class ASRAdapter(ASRAdapter_old):
    """ASR with external NPZ calibration (ORICA ``initialize_asr_from_npz_1``).

    ``transform()`` is identical to :class:`ASRAdapter_old`; ``fit()`` loads
    calibration from an NPZ file and uses receiver-style channel labels +
    ``standard_1020`` montage for asrpy.
    """
    ...
    def initialize_asr_from_npz_1(
        ...
        try:
            ...
            if self._backend == "asrpy":
                ...
                asr = MeegkitASR(sfreq=float(self._sfreq), cutoff=cutoff)
